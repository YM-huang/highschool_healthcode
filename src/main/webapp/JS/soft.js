var soft = {
                ...
                columns: [
                    {
                        field: 'title', title: '软件名称', width: 165, templet: function (item) {
                            ...
                            return '<span ' + click_opt + ' ' + sStyle + ' ><img '+ (item.type === 10?'style="height:20px;width:22px"':'') +' src=' + fName + '"/img/soft_ico/ico-.png">' + item.title + ' ' + version + '</span>';
                        }
                    },
                    {
    ...
    get_dep_list: function (p) {
        var loadT = layer.msg('正在获取列表 <img src="/img/ing.gif">', { icon: 16, time: 0, shade: [0.3, '#000'] });
        var pdata = {}
...
function AddDeployment(maction) {
    if (maction == 1) {
        ...
        var loadT = layer.msg('正在提交 <img src="/img/ing.gif">', { icon: 16, time: 0, shade: [0.3, '#000'] });
...
function AddSite(codename,title) {
    ...
    var loadT = layer.msg('正在创建站点 <img src="/img/ing.gif">', { icon: 16, time: 0, shade: [0.3, "#000"] })
    ...
    $.post('/site?action=AddSite', data, function (ret) {
        ...
        var loadT = layer.msg('<div class="depSpeed">正在提交 <img src="/img/ing.gif"></div>', { icon: 16, time: 0, shade: [0.3, "#000"] });
        ...
        $.post('/deployment?action=SetupPackage', pdata, function (rdata) {
            ...
            layer.open({
                ...
                content: "<div class='success-msg'>\
						<div class='pic'><img src='/img/success-pic.png'></div>\
						<div class='suc-con'>\
							" + sqlData + "\
						</div>\
					 </div>",
...
function GetSpeed() {
    ...
    $.get('/deployment?action=GetSpeed', function (speed) {
        ...
        if (speed.name == '下载文件') {
            speed = '<p>正在' + speed.name + ' <img src="/img/ing.gif"></p>\
...
var score = {
    ...
    get_score_info:function (obj,callback) {
        var loadT = layer.msg('<div class="depSpeed">正在获取评论信息 <img src="/img/ing.gif"></div>', { icon: 16, time: 0, shade: [0.3, "#000"] });
    ...
    set_score_info:function (obj,callback){
        var loadT = layer.msg('<div class="depSpeed">正在提交评论信息 <img src="/img/ing.gif"></div>', { icon: 16, time: 0, shade: [0.3, "#000"] });
