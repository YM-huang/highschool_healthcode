...
function bindBTPanel(a,type,ip,btid,url,user,pw){
	...
	if(a == 1) {
		...
		var btprofess = $("#btprofess").val();
		var data = "title="+btprofess+"&url="+encodeURIComponent(btaddress)+"&username="+btuser+"&password="+btpassword;
		if(btaddress =="" || btuser=="" || btpassword=="" || btprofess==""){
	...
	layer.open({
		...
		<div class='info-r'><input class='bt-input-text' type='text' name='btprofess' id='btprofess' value='"+ip+"' placeholder='"+lan.bt.panel_ps+"' style='width:100%'/></div>\
		</div>\
	...
	$("#btaddress").on("input",function(){
		...
		if(iptext) $("#btprofess").val(iptext[0]);
	}).blur(function(){
		var str =$(this).val();
		var isip = /([\w-]+\.){2,6}\w+/;
		var iptext = str.match(isip);
